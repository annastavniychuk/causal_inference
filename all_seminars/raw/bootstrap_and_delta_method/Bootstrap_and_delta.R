...
n<-NROW(mydata) #размер бутстраповской выборки совпадает с размером исходной выборки
tip<-NULL #цикл нужно с чего-то начать, поэтому создаём пустой вектор, куда попадут все оценки
tip
for (i in 1:B){
  bootID<-sample(c(1:n),n,replace = TRUE)	# случайная выборка с возвращением
  tip[i] = (-lm(mydata[bootID,1] ~ mydata[bootID,2] + I((mydata[bootID,2])^2))$coef[2])/(2*lm(mydata[bootID,1] ~ mydata[bootID,2] + I((mydata[bootID,2])^2))$coef[3])# бутстраповская статистика
  #для каждого i оценили модель и посчитали отношение оценок коэффициентов
  cat("\r", i, "of", B) #счётчик итераций в цикле
}
tip
plot(tip)
hist(tip, breaks=100) #распределение полученных оценок
